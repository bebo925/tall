...
<th {{ $attributes->merge(['class' => 'px-6 py-3 border-b border-gray-300'])->only('class') }}
    >
    @unless ($sortable)
    <span class="text-left flex text-xs leading-4 font-bold text-gray-800 uppercase tracking-wider">{{ $slot }}</span>
    @else
    <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-xs leading-4 font-bold text-gray-800 uppercase tracking-wider group focus:outline-none focus:underline">
        <span>{{ $slot }}</span>
